...
export default class LinkedList {
  ...
  getElementAt(index) {
    if (index >= 0 && index <= this.count) {
      ...
      for (let i = 0; i < index && current != null; i++) {
        // Percorre até alcançar o índex desejado
  ...
  removeAt(index) {
    if (index >= 0 || index < this.count) {
      // Verifica se o index é válido, varia de 0 ao tamanho da lista
      let current = this.head; // Current referencia o elemento atual, o mesmo q será removido
      // Remove o primeiro item
      if (index === 0) {
  ...
  toString() {
    if (this.isEmpty()) {
      // Se a lista estiver vazia, head será null ou undefined
      return "";
    }
